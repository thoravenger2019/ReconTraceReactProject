...
const DispenseSummaryReport = props => {
  console.log(props)
  ...
  const [onusacqtbl,setOnusAcqtbl]=useState(false);
...
*/
  ...
  const onShowDispense=async()=>{
    settblVisible(true);
    const validateFields=await form.validateFields();
    const values = form.getFieldsValue();
    console.log(values); 
    const dispenseReport = await axios.get(`getdispensesummaryreport/${values.clientID}/${values.channelID}/${values.modeID}/${values.terminalID}/${values.fromDateTxns}/${values.toDateTxns}/${values.txnType}`);
    ...
    const dataAll = dispenseReporttbl.map((item, index) => ({
      ...
      NFSISSUER:item.NFSISSUER,
      NFSISSUERDIFF:item.NFSISSUERDIFF,
      SWISSUER:item.SWISSUER,
  ...
  const issuercolumns = [
    ...
    {
      title: 'NFS Issuer Diff',
      dataIndex: 'NFSISSUERDIFF',
      key: 'NFSIssuerDiff',
    },
    {
      title: 'UnSettled Amount',
    ...
    <Layout>
      ...
      <Layout>
        ...
        <Layout style={{ height: "100vh", backgroundColor: "white" }}>
          <Content>
            <Card title="Dispense Summary Report" bordered={false} style={{ width: 1500 }}>
              <Form initialValues={{ remember: true }} layout={"vertical"} size={"large"} form={form}>
                ...
                <Row  gutter={8}>  
                  <Form.Item label=" " name="">             
                     <Button type={"primary"} size={"large"} style={{width:'100px'}} onClick={onShowDispense}>Show</Button>       
                  </Form.Item>           
                </Row>
              </Form>
              {tblVisible?(
              <Form>
              {onusacqtbl ? (
              <Table columns={columns} dataSource={dispenseSummaryReoprttbldata} bordered/>
              ):(
                <Table columns={issuercolumns} dataSource={dispenseSummaryReoprttbldata} bordered/>
              )}
              </Form>
              ):("")}
            </Card>
...
export default DispenseSummaryReport;